 ...
 * are all distinct.
 *
 * @see https://app.codility.com/programmers/lessons/7-stacks_and_queues/fish/
 */
class L07_02_Fish {
	 ...
	 * @param directions
	 * @return
	 * @see https://app.codility.com/demo/results/trainingMGHHU3-FQY/
	 */
	public int solution(int[] sizes, int[] directions) {
		int N = sizes.length;

		// this stack stores sizes of fishes going down
		...
		for (int i = 0; i < N; i++) {
			switch (directions[i]) {
			case DOWN_STREAM:
				downStreamStack.push(sizes[i]);
				break;
			case UP_STREAM:
			default:
				while (!downStreamStack.isEmpty()) {
					int other = downStreamStack.peek();
					if (other <= sizes[i]) { // the other fish is eaten by this one
						downStreamStack.pop(); // remove from stack
					} else { // this fish is eaten by the other one
						break;
					}
				}

				if (downStreamStack.isEmpty()) { // no more down stream fishes
					upStreamCount++;
				}
			}
		}

		return upStreamCount + downStreamStack.size();
	}
}
