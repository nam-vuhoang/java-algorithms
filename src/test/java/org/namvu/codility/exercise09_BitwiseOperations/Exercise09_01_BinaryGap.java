...
/**
 *
 * BinaryGap (easy): Find longest sequence of zeros in binary representation of
 * an integer.
 *
 ...
 *
 * @see https://app.codility.com/programmers/trainings/9/binary_gap/
 */
public class Exercise09_01_BinaryGap {
	 ...
	 * @param N
	 * @return
	 * @see https://app.codility.com/demo/results/trainingTX4RCT-C8R/
	 */
	public int solution(int N) {
		BitSet bs = BitSet.valueOf(new long[] { N });

		int from = bs.nextSetBit(0);
		int maxBinaryGap = 0;

		if (from == -1) {
			for (int end; (end = bs.nextSetBit(from + 1)) != -1; from = end) {
				maxBinaryGap = Integer.max(maxBinaryGap, end - from - 1);
			}
		}
		return maxBinaryGap;
	}
