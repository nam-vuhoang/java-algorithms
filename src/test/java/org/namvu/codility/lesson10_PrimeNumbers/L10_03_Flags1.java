...
class L10_03_Flags1 {
	...
	/**
	 * Test score: 66% (correctness: 100%, performance: 28%)
	 *
	 * @param A
	 * @return
	 */
	public int solution(int[] A) {
		...
		int maxDistance = peaks.get(peakCount - 1) - peaks.get(0);

		int hi = (int) Math.sqrt(maxDistance) + 1;
		int lo = 2;
	...
	private boolean check(List<Integer> peaks, int flagCount) {
		int lastPosition = Integer.MIN_VALUE;
		int minDistance = flagCount;
		for (int i = 0; i < peaks.size() && flagCount > 0; i++) {
			if (lastPosition + minDistance <= peaks.get(i)) {
				flagCount--;
