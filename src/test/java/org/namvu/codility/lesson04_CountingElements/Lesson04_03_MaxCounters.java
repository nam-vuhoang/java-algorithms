 ...
 */
public class Lesson04_03_MaxCounters {
	 ...
	 * @param A
	 * @return
	 * @see https://app.codility.com/demo/results/trainingMCYPT6-EQV/
	 */
	public int[] solution(int N, int[] A) {
		...
		for (int op : A) {
			int index = op - 1;
			if (index < N) { // operation: increase value at index
				counters[index] = Math.max(counters[index], minCount) + 1;
				maxCount = Math.max(counters[index], maxCount);
			} else { // operation: set all values to be current max
				minCount = maxCount;
			}
		}

		for (int index = 0; index < N; index++) {
			counters[index] = Math.max(counters[index], minCount);
		}
		return counters;
	}
