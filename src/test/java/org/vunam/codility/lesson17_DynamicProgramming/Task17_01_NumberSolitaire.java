...
/**
 * [MEDIUM] NumberSolitaire: In a given array, find the subset of maximal sum in
 * which the distance between consecutive elements is at most 6.
 *
 ...
 *
 * A[0] = 1 A[1] = -2 A[2] = 0 A[3] = 9 A[4] = -1 A[5] = -2 one possible game
 * could be as follows:
 *
 ...
 *
 * A[0] = 1 A[1] = -2 A[2] = 0 A[3] = 9 A[4] = -1 A[5] = -2 the function should
 * return 8, as explained above.
 *
 ...
 *
 * @see https://app.codility.com/programmers/lessons/17-dynamic_programming/number_solitaire/
 */
class Task17_01_NumberSolitaire {
	 ...
	 *
	 * https://app.codility.com/demo/results/trainingZZSXXG-6J3/
	 *
	 * @param A
	 * @return
	 */
	public int solution(int[] A) {
		...
		for (int i = 1; i < N; i++) {
			// get the maximum value among last 6 values before dp[i]
			int maxPrevious = Arrays.stream(dp, Math.max(0, i - 6), i).max().getAsInt();
