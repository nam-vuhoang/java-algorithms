 ...
 */
public class Lesson05_04_MaxAvgTwoSlice {

	/**
	 * Detected time complexity: O(N ** 2)
	 * Test result: 60% (Correctness: 100%, Performance: 20%)
	 *
	 * @param A
	 * @return
	 * @see https://app.codility.com/demo/results/training5728R8-PBE/
	 */
	public int solution(int[] A) {
		...
		// for each position find the min possible average using double loops
		double minAverage = Double.MAX_VALUE;
		int minAverageStart = 0;

		for (int start = 0; start < N - 1; start++) {
			for (int end = start + 1; end < N; end++) {
				int length = end - start + 1;
				double sum = prefixSums[end + 1] - prefixSums[start];
				double average = sum / length;
				if (minAverage > average) {
					minAverage = average;
					minAverageStart = start;
				}
			}
		}

		return minAverageStart;
	}
